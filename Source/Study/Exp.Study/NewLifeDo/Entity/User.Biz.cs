...
namespace Exp.Study.NewLifeDo.Entity
{
    ...
    public partial class User : Entity<User>
    {
        ...
        public override void Valid(Boolean isNew)
        {
            ...
            if (isNew && !Dirtys[__.CreatedOnUtc]) CreatedOnUtc = DateTime.Now;
        }
        ...
        {
			if (Addresss != null) Addresss.Delete();
        ...
        #region 扩展属性﻿
        [NonSerialized]
		private EntityList<Address> _Addresss;
		/// <summary>该User所拥有的Address集合</summary>
		[XmlIgnore]
		public EntityList<Address> Addresss
		{
			get
			{
				if (_Addresss == null && Id > 0 && !Dirtys.ContainsKey("Addresss"))
                {
					_Addresss = Address.FindAllByUserId(Id);
					Dirtys["Addresss"] = true;
				}
				return _Addresss;
			}
			set { _Addresss = value; }
